...
typedef struct {
    ...
    // Закреплённая группа (интеграции): стоит первой и рисуется на своей
    // подложке. `block_h` — высота всего блока от заголовка до последней
    // строки группы: подложку рисует заголовок, а размер её известен
    // только когда строки уже разложены.
    bool  pinned;
    int   block_h;
} PanelRow;
