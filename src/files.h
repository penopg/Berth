...
typedef struct {
    ...
    int    count;
    bool   exists;        // реестр есть; без него и раздела на странице нет
    bool   partial;       // строк в реестре больше, чем помещается
    time_t mtime;         // реестра при последнем чтении

    // Документы в папке, которых в реестре нет. -1 — ещё не считали.
    // Обход ограничен по числу записей: у проекта с чужим деревом внутри
    // (референсные исходники) он не должен стоить кадра.
    int    undescribed;
