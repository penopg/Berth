...
typedef struct {
    ...
    // Счёт с экрана Claude Code (крутилка «↓ N tokens»): идёт в реальном
    // времени, тогда как score из jsonl приходит по концу сообщения.
    long       screen, screen_from;
    ...
    long       hit_amount;       // сколько токенов ляжет в счёт в момент попадания
    float      since_hit;        // сколько герой не бил: без токенов бьёт сам, редко

    // Удары идут от счёта: разница между правдой (большее из счётчика
    // крутилки и записей jsonl) и показанным числом — очередь токенов, и
    // каждый удар кладёт в число свою порцию в момент попадания. Число
    // движется только ударами, поэтому подскок совпадает с ударом, а без
    // токенов не бывает ни того ни другого. Большая очередь — частые
    // короткие джебы, до десяти в секунду; маленькая — редкие полные удары.
...
void scene_screen(Scene *sc, long tokens);   // накопленный счётчик с экрана
