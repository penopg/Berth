              ...
              int cell_width, int cell_height)
{
    ...
    // Мастер pty не должен утекать в отвязанных помощников (генератор
    // вкладок, `open`, команды действий): унаследованный ими дескриптор
    // держал бы терминал вкладки открытым и после её закрытия.
    if (child > 0) fcntl(pty_fd, F_SETFD, FD_CLOEXEC);
    if (child == 0) {
