...
void scene_set(Scene *sc, SceneMood mood)
{
    ...
    case SCENE_COMPACT:
        // Уборка: герой ходит взад-вперёд один. Противник уходит — стоящий
        // в стойке рядом с гуляющим героем читался как сломанный бой; когда
        // сжатие кончится, бой начнётся заново с тем же счётом, и он вбежит
        // снова. Вбежавший наполовину разворачивается с места (ENEMY_LEAVE
        // идёт от текущего x).
        if (hero_ready(sc)) hero_act(sc);
        if (sc->enemy_phase == ENEMY_ENTER || sc->enemy_phase == ENEMY_FIGHT)
            enemy_leave(sc);
