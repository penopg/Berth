...
// Документ — то, что сделано для человека, а не для машины. Первая версия
// списка была уже: только читаемое. Но скринкаст, записанный агентом,
// человек тоже открывает и через неделю не помнит, что в нём, — а берт его
// не видел вовсе, потому что «.mp4» в список не входил.
